...
- Canvas 已使用“复制 Agent/MCP 上下文”这一宿主中立入口；当前功能分支已完成只读 Gateway、Storage v18 检索、local-Agent RAG、repository JSON CLI、Storage v19 统一写回账本，以及同一 Gateway 之上的十工具 MCP。新增的 `promptcard_delivery_preview`、`promptcard_delivery_commit`、`promptcard_delivery_status` 与 `promptcard_asset_stage` 对 Codex 和其他 MCP 宿主保持相同 schema 与结果；文件 staging 只允许配置的 workspace root 内真实文件，拒绝路径穿越和 symlink/junction 逃逸。外部 Agent 的 preview/commit 只产生待审阅提案；Prompt 接受后先保存全 `user` 节点并取得 `CVT-*`，图片则先经 30 MB、MIME、摘要和路径边界校验进入不透明 `AST-*` staging，再由 Canvas 保存普通图片节点并取得 `CVM-*` 后确认。两类流程都可安全重试，不重复建节点；图片写回不会伪造 provider generation run。Task 26A 的 Document create/change 已通过真实 Gateway、Storage 与浏览器 Canvas 进程闭环：严格解析闭合提案，Inbox 展示外部 Agent、精确 Skill revision、来源对象和请求身份；创建复用原生 Document AST，修改只按 `CVD/revision/digest` 定点转换为既有红删绿增建议，并在 Canvas 保存成功取得 `CVD-*` 后才确认 ledger。`CVD-*` / `CVS-*` 现可与文本、媒体一起进入用户明确创建的 `CVC-*`，浏览器读取时继续执行闭合结构校验。确定性 Bridge 标记、来源与 CVD 经服务重启仍可恢复；同请求重放只返回首个结果，不重复创建 Document。Task 26B 已完成首个 Storyboard 基础检查点：闭合 Gateway/Storage 写回、严格浏览器解析、原生序列适配和 Inbox 摘要已通过聚焦测试；Canvas 保存协调、逐字段真实审阅和重启/重放验收仍未完成，因此 `main` 在最终真实 Codex 总闭环验收前保持不变。
