...
- Canvas 的右侧栏现已将连接状态、受信 profile 与固定 scope、明确 PRJ/CVC、Bootstrap Skill、精确 Skill pin、Tool/写回能力、待审阅提案和近期失败集中为一个 **Agent 工作环境**。Bootstrap v2 由 Runtime 直接返回可执行上手说明，不再只是名字和 digest；它给出 fresh CVC、四字段 Skill pin、六类封闭写回形状及 preview→commit→人工审阅规则。创建 CVC 前会先强制持久化当前 Canvas，并使用 Storage 返回的权威项目 revision；无新编辑的空闲自动保存不再重复推进 revision。真实 Gateway/Storage/浏览器检查点、真实 Codex MCP 首次发现以及首个真实 Codex Document create→preview→commit→可视化接受→唯一 CVD 持久化切片均已通过。最终 Document 修改、Storyboard、Prompt、Codex 图片、重放和重启组成的四类总闭环仍是合入 `main` 的门槛。
