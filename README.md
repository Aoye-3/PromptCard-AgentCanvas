...
- Canvas 的右侧栏现已将连接状态、受信 profile 与固定 scope、明确 PRJ/CVC、Bootstrap Skill、精确 Skill pin、Tool/写回能力、待审阅提案和近期失败集中为一个 **Agent 工作环境**。用户切换 CVC 前必须通过 Storage 的项目归属与撤销检查；profile 选择只改变显示，不能替外部 Agent 授权。选择 CVC 中的 Document/Storyboard 等对象后，可以复制含精确 PRJ/CVC/对象引用和 Bootstrap-first 顺序的任务，不依赖截图或当前焦点猜测目标。真实 Gateway/Storage/浏览器检查点与真实 Codex MCP 首次发现均已通过：一个无 PromptCard 内部 schema 先验的 Codex 进程可依次发现 Runtime/Workspace，并读取精确 Skill、CVC 对象、Prompt 证据和图片素材。最终四类写回总闭环仍是合入 `main` 的门槛。
