...
interface BridgeDeliveryInboxProps {
  ...
  onAccept: (delivery: BridgePromptDelivery) => Promise<string[]>
}
...
export const BridgeDeliveryInbox = ({
  ...
  const [deliveries, setDeliveries] = useState<BridgePromptDelivery[]>([])
  ...
  const decide = async (delivery: BridgePromptDelivery, decision: 'accepted' | 'rejected') => {
    if (busyId) return
    ...
    try {
      ...
      if (decision === 'accepted' && resultCodes.length === 0) {
        setError('Prompt 尚未可靠保存，提案仍保留在待审阅队列。')
        return
      }
        ...
        {deliveries.map(delivery => (
          <article key={delivery.proposalId} className="rounded-lg border border-sky-200 bg-white p-3 shadow-sm">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <span className="text-[10px] font-semibold text-sky-700">
                  {delivery.visualProposal.agentName} · Prompt 创建
                </span>
              </div>
            ...
            <div className="mt-3 flex justify-end gap-2">
              <button
                type="button"
                aria-label="拒绝外部 Agent Prompt 提案"
              ...
              <button
                type="button"
                aria-label="接受外部 Agent Prompt 提案"
                disabled={busyId !== null}
