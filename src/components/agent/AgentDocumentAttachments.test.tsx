...
const RESOURCE_A = 'a'.repeat(32)
const RESOURCE_B = 'b'.repeat(32)
const RESOURCE_C = 'c'.repeat(32)
...
describe('AgentDocumentAttachments', () => {
  ...
  it.each([
    ...
    expect(mocks.upload).toHaveBeenCalledWith('project-1', file, expect.any(AbortSignal))
  ...
  it('accepts drag and drop, reports upload state, and removes only the message selection', async () => {
    ...
    expect(onUploadingChange).toHaveBeenLastCalledWith(1)
    expect(renderer.root.findByProps({ 'data-agent-document-dropzone': true }).props['aria-busy']).toBe(true)
    expect(renderer.root.findByProps({ 'aria-label': '添加项目文档' }).props.disabled).toBe(true)
    ...
    expect(onChange).toHaveBeenCalledWith([expect.objectContaining({ resourceId: RESOURCE_A })])
    ...
    act(() => renderer.update(
      <AgentDocumentAttachments
        projectId="project-1"
        attachments={[{
          resourceId: RESOURCE_A, name: 'plan.md', contentType: 'text/markdown',
  ...
  it('reports per-file progress while persisting a batch', async () => {
    ...
    mocks.upload
      .mockResolvedValueOnce(uploadedResource({ id: RESOURCE_A, originalFilename: 'one.md' }))
    ...
    await act(async () => {
      resolveSecond(uploadedResource({ id: RESOURCE_B, originalFilename: 'two.md' }))
      await uploadPromise
    })
  })
  ...
  it('ignores an upload that finishes after the project identity changes', async () => {
    ...
    expect(mocks.upload.mock.calls[0]?.[2]).toBeInstanceOf(AbortSignal)
    expect(mocks.upload.mock.calls[0]?.[2].aborted).toBe(true)
    await act(async () => {
      resolveUpload(uploadedResource())
      await uploadPromise
    })
  ...
  it('admits only one active picker or drop batch', async () => {
    let resolveUpload!: (value: ReturnType<typeof uploadedResource>) => void
    mocks.upload.mockReturnValueOnce(new Promise(resolve => { resolveUpload = resolve }))
    let renderer!: ReactTestRenderer
    act(() => {
    ...
    act(() => {
      firstBatch = renderer.root.findByProps({ 'data-agent-document-dropzone': true }).props.onDrop({
        preventDefault: vi.fn(),
        dataTransfer: { files: [new File(['one'], 'one.md', { type: 'text/markdown' })] }
      })
      void renderer.root.findByProps({ 'aria-label': '添加项目文档' }).props.onChange({
        currentTarget: {
          files: [new File(['two'], 'two.md', { type: 'text/markdown' })],
          value: 'selected'
        }
      })
    })
  ...
  it.each([0, 1])('continues a three-file batch after file %i fails and merges successes deterministically', async (failureIndex) => {
    const results = [
      uploadedResource({ id: RESOURCE_C, originalFilename: 'three.md' }),
      uploadedResource({ id: RESOURCE_B, originalFilename: 'two.md' }),
      uploadedResource({ id: RESOURCE_A, originalFilename: 'one.md' })
    ]
    results.forEach((resource, index) => {
      if (index === failureIndex) mocks.upload.mockRejectedValueOnce(new Error(`failed-${index}`))
      else mocks.upload.mockResolvedValueOnce(resource)
    })
    const onChange = vi.fn()
    let renderer!: ReactTestRenderer
    act(() => {
    ...
    const expectedIds = results
      .filter((_resource, index) => index !== failureIndex)
      .map(resource => resource.id)
      .sort()
    expect(onChange).toHaveBeenLastCalledWith(expectedIds.map(resourceId => (
      expect.objectContaining({ resourceId })
    )))
    expect(renderer.root.findByProps({ role: 'alert' }).children.join('')).toContain(`failed-${failureIndex}`)
  })

  it('deduplicates repeated resource identities returned inside one batch', async () => {
    mocks.upload
      .mockResolvedValueOnce(uploadedResource({ originalFilename: 'one.md' }))
      .mockResolvedValueOnce(uploadedResource({ originalFilename: 'duplicate.md' }))
    const onChange = vi.fn()
    let renderer!: ReactTestRenderer
    act(() => {
      renderer = create(
    ...
    expect(onChange).toHaveBeenLastCalledWith([
      expect.objectContaining({ resourceId: RESOURCE_A, name: 'duplicate.md' })
    ])
  })

  it('rejects more than five documents and an aggregate over 100 MiB before upload', async () => {
    const attachments = Array.from({ length: 5 }, (_, index) => ({
      resourceId: index.toString(16).padStart(32, '0'),
      name: `file-${index}.pdf`,
  ...
  it('shows a safe upload error without creating a selected attachment', async () => {
    ...
    expect(onChange).not.toHaveBeenCalled()
    expect(renderer.root.findByProps({ role: 'alert' }).children.join('')).toContain('文档格式无效')
  })
})
