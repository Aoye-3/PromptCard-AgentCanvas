...
2. Check that the storage health proxy and direct storage health both succeed:
   ...
   The response must report `serviceVersion: "2.0.0"` and `schemaVersion: 16`. `start.bat` and `start-desktop.bat` both reach the same combined service launcher, which rejects an older Storage schema instead of opening the frontend against it. If a stale process is still listening, inspect `logs/storage-service.err.log` and rerun the visible launcher rather than changing the expected schema in the script.
