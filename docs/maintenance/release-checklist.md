...
- Confirm PromptCard Storage health reports schema v15, including `agentConversations`, `skillHub`, `contextPacks`, canonical Skill packages, exact-revision trust reviews, and independent host pins, before enabling the current application surface
- Block a combined-launcher release until the legacy exact-schema-v9 checks in `agent:check` and `start-dev-with-agent.ps1` are migrated to the v15 readiness contract
