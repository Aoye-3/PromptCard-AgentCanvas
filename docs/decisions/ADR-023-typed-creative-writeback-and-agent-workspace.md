...
The final real-Codex loop harness exposed boundary defects that isolated fixtures had not exercised: four-field Skill pins were reduced in downstream layers, Bootstrap was identity-only, a delayed no-op Canvas save could stale a new CVC, a new host could not safely derive Document byte ranges/digests or the exact Document/Storyboard outer payloads, Bridge Prompt provenance omitted the closed model `capabilities` field, modifier-click was collapsed back to one Canvas selection, and image staging was not executable from Bootstrap alone. All runtime layers now consume the normative four-field pin; Workspace fails stale CVC at discovery; CVC creation flushes Canvas state while no-op idle saves are suppressed; exact CVD resolution emits bounded `documentEditEvidence`; Bootstrap v6 freezes the executable Document, Storyboard, staging, and image-placement envelopes; Prompt save responses pass strict provenance normalization; and modifier selection can create an explicit multi-object CVC without widening authority. Seven newly connected Codex sessions with no preloaded PromptCard schema completed Document create/change, Storyboard create/change, Prompt create, actual Codex image generation, staging, and exact-shot placement through the existing native review UIs in 8.2 minutes. The accepted Storyboard preserved exact source-Document evidence and converted external row ordinal 0 into private row identity; the accepted Prompt produced one all-`user` `CVT-*`; the accepted image produced one `CVM-*` with the Prompt as source and Storyboard shot 0 as target only after durable save and ledger acknowledgement. Storage confirmed that this external image created no provider generation run. Four-kind replay, conflict evidence, and process restart remain in the final gate.
