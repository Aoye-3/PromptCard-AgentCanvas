...
The final real-Codex loop harness exposed three cross-layer defects that isolated fixtures had not exercised. First, Bridge v3 and MCP correctly required `projectionHealth`, while Gateway, Storage, and the browser still accepted only the other three Skill-pin fields. Second, the built-in Bootstrap was returned only as an identity, leaving a new Agent to infer a complex closed union. Third, opening a Canvas could leave a delayed idle save that advanced the project revision after a user created a CVC. All runtime layers now consume the normative four-field pin; Runtime returns Bootstrap v2 executable instructions; Workspace fails stale CVC at discovery; and CVC creation flushes Canvas state before snapshotting while no-op idle saves are suppressed. An actual Codex then completed Runtime → Workspace → exact Skill/source → Document preview/commit → visual acceptance in 2.2 minutes and produced one durable CVD with exact profile, source, request, and Skill provenance. This accepts the first Document-create slice only; the total four-kind/replay/restart gate remains open.
