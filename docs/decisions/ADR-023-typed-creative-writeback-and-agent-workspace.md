...
Task 26A's third checkpoint implements the browser/Canvas adapter. The Storage client fails closed on unknown fields, internal targets, mismatched CVD/base/proposal data, or divergent provenance. The Inbox exposes Agent, exact Skill revision, source references, request identity, and create/change intent. Acceptance creates one deterministic native Document or converts exact CVD-targeted operations into the existing tracked suggestion representation; it does not acknowledge the ledger until Canvas persistence returns one CVD. A Canvas-side Bridge marker detects retries and tampering, while the public CVD is preserved by node normalization so restart does not erase external authority. At that checkpoint Task 26A remained open for the real process-level preview/commit/browser/restart probe; the later Document checkpoint supplied that evidence before Storyboard implementation began.

The final real-Codex loop harness exposed boundary defects that isolated fixtures had not exercised: four-field Skill pins were reduced in downstream layers, Bootstrap was identity-only, a delayed no-op Canvas save could stale a new CVC, a new host could not safely derive Document byte ranges/digests or the exact Document/Storyboard outer payloads, Bridge Prompt provenance omitted the closed model `capabilities` field, modifier-click was collapsed back to one Canvas selection, image staging was not executable from Bootstrap alone, and the legacy 20×16 Canvas offset allowed large Bridge-created nodes to overlap. All runtime layers now consume the normative four-field pin; Workspace fails stale CVC at discovery; CVC creation flushes Canvas state while no-op idle saves are suppressed; exact CVD resolution emits bounded `documentEditEvidence`; Bootstrap v6 freezes the executable Document, Storyboard, staging, and image-placement envelopes; Prompt save responses pass strict provenance normalization; modifier selection can create an explicit multi-object CVC without widening authority; and external Bridge create/place paths use conservative collision-free slots without changing ordinary manual placement. Seven newly connected Codex sessions with no preloaded PromptCard schema completed Document create/change, Storyboard create/change, Prompt create, actual Codex image generation, staging, and exact-shot placement through the existing native review UIs. The accepted Storyboard preserved exact source-Document evidence and converted external row ordinal 0 into private row identity; the accepted Prompt produced one all-`user` `CVT-*`; the accepted image produced one `CVM-*` with the Prompt as source and Storyboard shot 0 as target only after durable save and ledger acknowledgement. Storage confirmed that this external image created no provider generation run.

The same persisted requests subsequently passed a real stop/restart boundary. A new Codex rediscovered the environment and exact four-object CVC, replayed the identical asset-stage request and all six preview/commit/status sequences, and observed the existing accepted results. Reusing the same request key with a different digest returned `delivery_conflict`; a new browser restored the unique `CVD-*`, `CVS-*`, `CVT-*`, and `CVM-*`, their provenance, and their accepted state. Successful staging is externally reported as durable state `accepted` with only an opaque handle and digest; no internal asset ID or path crosses the Bridge. This evidence completes the ADR's typed-delivery recovery decision while leaving Phase 6 hardening and distribution gates open.
