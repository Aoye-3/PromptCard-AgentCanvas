...
The final real-Codex loop harness exposed boundary defects that isolated fixtures had not exercised: four-field Skill pins were reduced in downstream layers, Bootstrap was identity-only, a delayed no-op Canvas save could stale a new CVC, a new host could not safely derive Document byte ranges/digests or the exact Document/Storyboard outer payloads, and Bridge Prompt provenance omitted the closed model `capabilities` field. All runtime layers now consume the normative four-field pin; Workspace fails stale CVC at discovery; CVC creation flushes Canvas state while no-op idle saves are suppressed; exact CVD resolution emits bounded `documentEditEvidence`; Bootstrap v5 freezes the executable Document and Storyboard envelopes; and Prompt save responses pass the same strict provenance normalization used on restart. Five newly connected Codex sessions with no preloaded PromptCard schema then completed Document create/change, Storyboard create/change, and Prompt create through the existing native review UIs in 6.4 minutes. The accepted Storyboard preserved exact source-Document evidence and converted external row ordinal 0 into private row identity; the accepted Prompt produced one all-`user` `CVT-*` only after durable save and ledger acknowledgement. Generated image, four-kind replay, and process restart remain in the final gate.
