...
The final real-Codex loop harness exposed boundary defects that isolated fixtures had not exercised: four-field Skill pins were reduced in downstream layers, Bootstrap was identity-only, a delayed no-op Canvas save could stale a new CVC, and a new host could not safely derive Document byte ranges/digests or the `payload.operations` wrapper. All runtime layers now consume the normative four-field pin; Workspace fails stale CVC at discovery; CVC creation flushes Canvas state while no-op idle saves are suppressed; exact CVD resolution emits bounded `documentEditEvidence`; and Bootstrap v4 freezes the executable Document-change envelope. An actual Codex with no preloaded PromptCard schema then completed Document create and visual acceptance, opened a fresh exact-CVD CVC, proposed an exact-base full-block change, and passed the existing red-delete/green-add review through final persistence in 2.9 minutes. This accepts the real-host Document create/change slice only; Storyboard, Prompt, generated image, four-kind replay, and process restart remain in the final gate.
