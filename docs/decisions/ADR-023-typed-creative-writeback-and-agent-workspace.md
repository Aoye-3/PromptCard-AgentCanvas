...
The final real-Codex loop harness exposed boundary defects that isolated fixtures had not exercised: four-field Skill pins were reduced in downstream layers, Bootstrap was identity-only, a delayed no-op Canvas save could stale a new CVC, and a new host could not safely derive Document byte ranges/digests or the exact Document/Storyboard outer payloads. All runtime layers now consume the normative four-field pin; Workspace fails stale CVC at discovery; CVC creation flushes Canvas state while no-op idle saves are suppressed; exact CVD resolution emits bounded `documentEditEvidence`; and Bootstrap v5 freezes the executable Document and Storyboard envelopes. Four newly connected Codex sessions with no preloaded PromptCard schema then completed Document create/change and Storyboard create/change through the existing red-delete/green-add and per-field review UIs in 5.2 minutes. The accepted Storyboard preserved exact source-Document evidence and converted external row ordinal 0 into private row identity before native review. This accepts the real-host Document and Storyboard slices only; Prompt, generated image, four-kind replay, and process restart remain in the final gate.
