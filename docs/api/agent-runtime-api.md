...
`workspace` requires an explicit project/context pair, lists only objects snapshotted into that CVC, and resolves only active, trusted, enabled exact Codex Skill pins in the profile's configured repository scope. `reference` refuses codes outside the CVC. `prompt-search` reuses Storage v18 retrieval with the trusted Bridge profile as caller identity. `asset` accepts only a `PLM` or `CVM` explicitly contained by the CVC, limits the body to 5 MiB, validates current lifecycle/MIME/size/reference metadata, and returns filename, MIME, size, SHA-256 and Base64 without an internal asset ID or path. Delivery preview/commit/status use the profile-scoped v19 ledger; Prompt and image kinds are end-to-end implemented. Gateway accepts the closed v3 `document.create` and `document.change` unions, requires `bridge:deliver:document`, canonicalizes exact CVC/CVD/source references, validates approved Skill pins, routes preview/commit only to the protected Document Storage adapter, and keeps status read-only through the shared ledger. Absent optional fields are omitted before forwarding so the Gateway payload remains byte-shape compatible with the closed Storage contract. The browser parses the same closed Document records, displays Agent/Skill/source/request provenance, and applies them only through native Document AST/suggestion persistence before returning one Storage-owned `CVD-*` to the ledger. Task 26A's real-process and restart/replay acceptance is complete; Storyboard implementation is Task 26B.
