...
`runtime` returns Bootstrap v2 with its executable `instructions`, not merely a name and digest. The guide tells a new host to discover the exact Workspace, stop on stale/revoked context, copy the closed four-field Skill pins, use the six kind-specific target/payload shapes, preview before commit, and stop at visual review; `promptcard-bootstrap` is built in and is not an `SKL-*` readable through `skill_read`.
