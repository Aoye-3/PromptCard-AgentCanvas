...
Plan 008 Tasks 15.6-15.10 are implemented and technically accepted as the regression baseline. Checkpoint 3.5 manual probes are merged into final real-Codex acceptance. Tasks 16-25 now provide the bounded Bridge read surface, deterministic JSON CLI, Storage v18 transactional Prompt retrieval core, local-Agent RAG, CVC-authorized media reads, repository-owned STDIO/loopback HTTP MCP, the durable Storage v19 delivery ledger, reviewable additive Prompt writeback, and bounded staged-image placement on `feat/skill-document-storyboard-loop`; Task 26 MCP delivery/status exposure is next.
