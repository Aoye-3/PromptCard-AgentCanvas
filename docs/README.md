...
- Keep image-generation conversations, runs, placements, image derivations, text-Agent conversations, per-conversation model bindings, public references, context packs, canonical Skill revisions, and host pins in the current PromptCard Storage schema v14. Do not embed image runs or Agent transcripts in project JSON, and do not use browser storage or host projections as authoritative state.
