...
**Final-loop Document checkpoint (2026-08-31):** the actual-Codex Document create/change slice passes. The harness creates a real project and trusted Skill pin, flushes Canvas state into an explicit fresh CVC, then starts newly connected Codex processes with no PromptCard schema preloaded. The first follows Runtime Bootstrap, Workspace, exact Skill/source, preview/commit and visual acceptance to produce one durable CVD. The second opens a fresh CVC containing that exact CVD, resolves its revision/digest plus bounded block text/UTF-8 length/digest evidence, submits one exact-base `payload.operations` replacement, and stops at review. Chromium proves native red-delete/green-add rendering, accepts all revisions, and verifies the final text with zero remaining suggestions. The passing run completed in 2.9 minutes. RED runs exposed and fixed four-field Skill-pin drift, identity-only Bootstrap, late no-op Canvas save staling a CVC, missing stale discovery, absent Document edit evidence, and an ambiguous outer operation field. Bootstrap v4 and exact-CVD evidence now make the path executable without schema guessing or model-side hashing. Focused gates pass (contracts 52, Gateway/environment 37, Bootstrap/frontend 107, MCP 10, TypeScript and touched lint). Storyboard, Prompt, Codex-generated image, four-kind replay, and restart follow. This checkpoint does not satisfy Checkpoint 5 and does not move `main`.
