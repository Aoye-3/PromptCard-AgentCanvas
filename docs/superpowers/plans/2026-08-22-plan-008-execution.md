...
**Final-loop checkpoint (2026-08-31):** the first actual-Codex writeback slice passes. The harness creates a real project and trusted Skill pin, requires the user-facing Canvas flow to persist current state and create an explicit fresh CVC, then starts a newly connected Codex with no PromptCard schema preloaded. Codex follows Runtime Bootstrap v2, discovers Workspace, reads the exact Skill/source, submits a closed Document preview and commit, and stops at review. Chromium accepts the proposal and proves one durable CVD, exact text, profile, CVC, source, request, and Skill provenance. The final passing run completed in 2.2 minutes. Earlier RED runs exposed and fixed four-field Skill-pin drift, a non-executable Bootstrap identity, late Canvas autosave staling a newly created CVC, and missing stale detection at Workspace discovery. Focused gates pass (contracts 52, Gateway/environment 36, CVC/Canvas 110, Bootstrap/frontend 107, MCP 10, TypeScript and touched lint). The next slice is exact-base Document change through native tracked suggestions; Storyboard, Prompt, Codex-generated image, replay, and restart follow. This checkpoint does not satisfy Checkpoint 5 and does not move `main`.
