...
PromptCard Storage is the sole durable authority for project text-Agent history. Schema v9 introduced the current nullable `agent_conversations.model_binding_json` on top of the v8 conversation rows; schema v15 remains the active database version. Gateway loads that history and binding, validates the owning project, entrypoint, mode, permission scope, model whitelist, and current connection readiness before calling pi. Media analysis is intentionally temporary and never writes conversation rows.
