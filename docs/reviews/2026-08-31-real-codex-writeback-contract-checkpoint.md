...
- Scope: real-Codex Document/Storyboard create/change, Prompt create, generated image placement, and Bridge v3 first-contact usability
- Merge state: feature branch only; `main` remains unchanged
- Acceptance state: real Codex four-output review chain passed; unified replay, digest conflict, and restart remain open
...
- Bootstrap revision 6 is returned with executable first-contact instructions covering fresh CVC discovery, exact Skill pins, six writeback shapes, exact Document operations, complete Storyboard rows/changes, six-field bounded image staging, exact-shot placement, preview/commit, and visual review.
...
- Explicit modifier multi-selection to two-object CVC preview: 1 passed in 42.5 seconds.
- Actual Codex Document/Storyboard/Prompt/generated-image chain with all native review UIs, exact-shot placement, final persistence, and zero provider generation runs: 1 passed in 8.2 minutes across seven newly connected Codex sessions.
...
The Document, Storyboard, Prompt, and generated-image slices are accepted because actual Codex proposals created exactly one `CVD`, one exact-source `CVS`, one create-only `CVT`, and one `CVM`, then changed both mutable exact bases through the native red-delete/green-add and per-field review UIs. The user-facing flow persisted the Document replacement with zero suggestions, the Storyboard camera `Close-up` with zero pending field changes, the exact Prompt as all-`user` segments, and the generated image with complete Bridge provenance, Prompt source, exact Storyboard-shot target, and no provider generation run. The strict harness recorded no failed PromptCard Tool call in the passing run. This is not Checkpoint 5.

Extend the same real-host fixture with replay of every delivery, same-key/different-digest conflict evidence, and process restart recovery before final gates and `main` merge.
